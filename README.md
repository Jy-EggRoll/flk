...
- 0.3：开发国际化功能，为软件内所有字符串添加抽象 i18n 层（已实现：CLI 与 WebUI 均支持中英切换，默认英文）
...
`serve config` 打开的 WebUI 同样会跟随当前语言。翻译缺失时自动回退到英文原文，不会出现空白或乱码。

### 非交互模式（--yes）

flk 的确认默认需要真实终端。为便于脚本、CI 等无人值守场景，所有需要确认的命令都支持全局 `--yes`（简写 `-y`）：

- 启用后，所有确认一律自动同意，且完全不读取终端，不会因为缺少交互终端而挂起
- 未启用 `--yes` 且标准输入不是终端时，flk 不再等待输入，而是立即报错并提示改用 `--yes`，从根本上避免“卡死”
- 对 `fix`、`unlink` 这类逐项交互命令，`--yes` 等价于 `--all`，即自动处理全部记录
