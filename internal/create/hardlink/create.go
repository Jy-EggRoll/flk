...
func Create(primPath, secoPath string, force bool) error {
	...
	if force {
		...
		if info, err := os.Lstat(secoPath); err == nil { // 文件/链接存在
			logger.Debug("secoPath 对应的文件/链接存在，模式: " + info.Mode().String())
			if err := os.Remove(secoPath); err == nil {
				logger.Info("已成功删除 secoPath 对应的文件/链接")
			} else {
				logger.Error("删除失败: " + err.Error())
				return err
			}
		} else {
			logger.Debug("secoPath 对应的文件不存在，无需删除，错误: " + err.Error())
		}
